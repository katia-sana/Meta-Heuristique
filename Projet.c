...
Pnoeud Placer (Pnoeud p, int numf){//p : le pointeur sur le noeud générateur (le noeud père), numf : le numéro du fils [0 .. n-1]
       ...
       for (i = 0 ; i < p -> niveau ; i++){
           fils -> t[i] = p -> t[i]; //héritage des valeurs du tableau t du père
       }
       fils -> t[p -> niveau] = numf; //le numéro du fils : numf, correspond à la colonne où on compte placer la nouvelle reine
...
main (){   
     ...
     //traitement  
     Pnoeud fils, p = (Pnoeud)malloc(sizeof(noeud)); //création du noeud initial
     p -> t = NULL;
     p -> niveau = 0;
